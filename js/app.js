    /* ===================== 프로젝트 전역 상태 ===================== */
    let _currentProject = null; // { id, name, description, start_date, end_date }
    const GEN_PROJECT_KEY = 'WBS_GEN_CURRENT_PROJECT_ID';
    let _authCallback = null; // requireAuthThen 용 콜백
    ...
    /* === wbs_gen_analyses 기반 저장/조회 (프로젝트 범용) === */

    function supabaseAddOrUpdate(record, existingId = null) {
      if (!_currentProject) return Promise.reject(new Error('프로젝트를 먼저 선택하세요.'));
      ...
      const save = existingId
        ? client.from('wbs_gen_analyses').update(dbRecord).eq('id', existingId).select('id')
        : client.from('wbs_gen_analyses').insert(dbRecord).select('id');

      return save.then(({ data, error }) => {
        if (error) throw error;
        if (data && data.length > 0) return data[0].id;
        return client.from('wbs_gen_analyses').insert(dbRecord).select('id').then(({ data: inserted, error: insertError }) => {
    ...
    function getCombinedHistoryList() {
      if (!_currentProject) return Promise.resolve([]);
      const client = getSupabaseClient();
      if (!client) {
        return genDbGetAll().then(all => all.filter(r => r.project_id === _currentProject.id));
      }
      return client.from('wbs_gen_analyses')
        .select('id, saved_at, wbs_date, file_name, summary')
        .eq('project_id', _currentProject.id)
        .order('id', { ascending: false })
        .then(({ data, error }) => {
          if (error) {
            console.error('Supabase gen_analyses select error:', error);
            return genDbGetAll().then(all => all.filter(r => r.project_id === _currentProject.id));
          }
          return (data || []).map(row => ({
            id: row.id, savedAt: row.saved_at, wbsDate: row.wbs_date,
            fileName: row.file_name, summary: row.summary
          }));
        });
    }
    ...
    function isDelayByDate(t) {
      const end = parseDate(t.end);
      // 기준일 당일 마감 업무는 "진행중"으로 처리 (당일 자정 이후로 넘겨야 지연)
...
<div class="tree-phase ${isOpen}" id="ph_${phase.replace(/\s/g,'_')}">
  ...
  <div class="tree-phase-body">`;
    ...
    function requireAuth(pageId) {
      ...
      _pendingPage = pageId;
      _authCallback = null;
      _openAuthModal('구성원 관리와 메일 보내기 메뉴는 관리자 확인 후 이용할 수 있습니다.');
    }

    function requireAuthThen(callback, desc) {
      if (_authed) { callback(); return; }
      _pendingPage = '';
      _authCallback = callback;
      _openAuthModal(desc || '이 기능은 관리자 확인 후 이용할 수 있습니다.');
    }
    ...
    function submitAuth() {
      ...
      if (val === '4343') {
        _authed = true;
        closeAuthModal();
        if (_authCallback) { const fn = _authCallback; _authCallback = null; fn(); }
        else if (_pendingPage) navigateTo(_pendingPage);
      } else {
        const error = document.getElementById('authError');
    ...
    // GenDB 우선, 없으면 Supabase에서 tasks 포함 전체 레코드 fetch
    function fetchFullRecord(id) {
      return genDbGet(id).then(local => {
        if (local && local.tasks) return local;
        const client = getSupabaseClient();
        if (!client) return local || null;
        return client.from('wbs_gen_analyses').select('*').eq('id', id).maybeSingle()
          .then(({ data, error }) => {
            if (error || !data) return local || null;
            return {
              id: data.id, savedAt: data.saved_at, wbsDate: data.wbs_date,
              fileName: data.file_name, tasks: data.tasks, summary: data.summary
            };
          });
      });
    }
    ...
    function parseWbsWorkbook(workbook) {
      ...
      const actName  = workbook.SheetNames.find(n => n.includes('실적') || n.includes('실행'));
    ...
    /* === wbs_gen_members 기반 구성원 CRUD (프로젝트 범용) === */
    function mdbGetAll() {
      if (!_currentProject) return Promise.resolve([]);
      const client = getSupabaseClient();
      if (!client) {
        return genMembersDbGetAll().then(all => all.filter(r => r.project_id === _currentProject.id));
      }
      return client.from('wbs_gen_members').select('*')
        .eq('project_id', _currentProject.id).order('name', { ascending: true })
        .then(({ data, error }) => {
          if (error) {
            console.error('Supabase gen_members select error:', error);
            return genMembersDbGetAll().then(all => all.filter(r => r.project_id === _currentProject.id));
          }
          return (data || []).map(row => ({
            id: row.id, name: row.name, role: row.role, email: row.email,
            phone: row.phone, title: row.title, createdAt: row.created_at
          }));
        });
    }
    ...
    function mdbAdd(record) {
      ...
      const dbRecord = {
        project_id: _currentProject.id,
        name: record.name, role: record.role, email: record.email,
        phone: record.phone, title: record.title, created_at: record.createdAt
      };
      if (!client) {
        return genMembersDbAdd(Object.assign({}, record, { project_id: _currentProject.id }));
      }
      return client.from('wbs_gen_members').insert(dbRecord).select('id')
        .then(({ data, error }) => { if (error) throw error; return data[0].id; });
    }
    ...
    function mdbPut(id, record) {
      ...
      if (!client) {
        return genMemberDbPut(id, Object.assign({}, record, { project_id: _currentProject?.id }));
      }
      return client.from('wbs_gen_members').update(dbRecord).eq('id', id).select('id')
        .then(({ data, error }) => { if (error) throw error; return id; });
    }

    function mdbDelete(id) {
      const client = getSupabaseClient();
      if (!client) {
        return genMemberDbDelete(id);
      }
      return client.from('wbs_gen_members').delete().eq('id', id)
    ...
    function saveCurrentAnalysis() {
      ...
      if (!_currentProject) {
        showToast('프로젝트를 선택해야 클라우드에 저장됩니다. (관리자 로그인 필요)', 'err'); return;
      }
      const planRate = calcPlanRate();
      const actualRate = calcActualRate();
      const record = {
      ...
      if (client) {
        getCombinedHistoryList().then(all => {
          ...
          supabaseAddOrUpdate(record, existingId).then(globalId => {
            ...
            genDbGetAll().then(localAll => {
              const localExisting = localAll.find(r => r.fileName === CURRENT_SOURCE_NAME && r.project_id === _currentProject?.id);
              const recWithPid = Object.assign({}, record, { project_id: _currentProject?.id });
              const saveLocal = localExisting ? genDbUpdate(localExisting.id, recWithPid) : genDbAdd(recWithPid);
              return saveLocal.then(id => {
                showToast('클라우드 저장 실패 — 브라우저에만 저장됨: ' + err.message, '');
                updateHistoryCount();
        ...
        genDbGetAll().then(all => {
          ...
          if (existing) {
            ...
            genDbUpdate(existing.id, recWithPid).then(id => {
              showToast(localMsg, 'ok');
              updateHistoryCount();
              populateFileSelect(id);
              renderHistoryPage();
              updateUrlParam(id);
    ...
    function initSupabaseEvents() {
      ...
      if (btnSave) {
        btnSave.addEventListener('click', () => {
          ...
          if (testClient) {
            testClient.from('wbs_gen_projects').select('id').limit(1).then(({ error }) => {
              if (error) {
...
<div class="hist-card${isActive ? ' active-snapshot' : ''}">
  ...
  <div class="hist-footer">
      ...
      genDbGet(id).then(r => {
        if (r && r.tasks) {
          ...
          if (client) {
            ...
            client.from('wbs_gen_analyses').select('*').eq('id', id).maybeSingle().then(({ data, error }) => {
              if (error) throw error;
              if (!data) { showToast('저장 레코드를 찾을 수 없습니다.', 'err'); return; }
              if (_currentProject && data.project_id !== _currentProject.id) {
                updateUrlParam(null);
                navigateTo('p-upload');
                return;
              }
              const fileData = data.file_data ? base64ToArrayBuffer(data.file_data) : null;
              const record = {
                id: data.id, savedAt: data.saved_at, wbsDate: data.wbs_date,
                fileName: data.file_name, fileData, tasks: data.tasks, summary: data.summary
              };
              genDbUpdate(data.id, record).catch(() => {});
      ...
      genDbGet(id).then(r => {
        if (r && r.fileData) {
          ...
          if (client) {
            ...
            client.from('wbs_gen_analyses').select('file_name, file_data').eq('id', id).maybeSingle()
              .then(({ data, error }) => {
                if (error) throw error;
                if (!data || !data.file_data) { showToast('저장된 파일 데이터가 없습니다.', 'err'); return; }
                triggerFileDownload(data.file_name, base64ToArrayBuffer(data.file_data));
    ...
    function deleteAnalysis(id) {
      genDbGet(id).then(r => {
        ...
        if (client) {
          ...
          client.from('wbs_gen_analyses').delete().eq('id', id)
            .then(({ error }) => { if (error) throw error; doDelete(); })
    ...
    /* ===================== Gen IndexedDB (로컬 캐시) ===================== */
    const GEN_DB_NAME = 'WbsGenDB', GEN_DB_VER = 1;
    let _genDb = null;

    function initGenDB() {
      return new Promise((resolve, reject) => {
        if (_genDb) { resolve(_genDb); return; }
        const req = indexedDB.open(GEN_DB_NAME, GEN_DB_VER);
        req.onupgradeneeded = e => {
          const d = e.target.result;
          if (!d.objectStoreNames.contains('gen_analyses')) {
            const s = d.createObjectStore('gen_analyses', { keyPath: 'id', autoIncrement: true });
            s.createIndex('project_id', 'project_id', { unique: false });
          }
          if (!d.objectStoreNames.contains('gen_members')) {
            const s = d.createObjectStore('gen_members', { keyPath: 'id', autoIncrement: true });
            s.createIndex('project_id', 'project_id', { unique: false });
          }
        };
        req.onsuccess = e => { _genDb = e.target.result; resolve(_genDb); };
        req.onerror = e => reject(e.target.error);
      });
    }

    function genDbGet(id) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_analyses', 'readonly').objectStore('gen_analyses').get(id);
        req.onsuccess = e => resolve(e.target.result);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genDbGetAll() {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_analyses', 'readonly').objectStore('gen_analyses').getAll();
        req.onsuccess = e => resolve(e.target.result);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genDbAdd(record) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_analyses', 'readwrite').objectStore('gen_analyses').add(record);
        req.onsuccess = e => resolve(e.target.result);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genDbUpdate(id, record) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const rec = Object.assign({}, record, { id });
        const req = d.transaction('gen_analyses', 'readwrite').objectStore('gen_analyses').put(rec);
        req.onsuccess = () => resolve(id);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genDbDelete(id) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_analyses', 'readwrite').objectStore('gen_analyses').delete(id);
        req.onsuccess = () => resolve();
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genMembersDbGetAll() {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_members', 'readonly').objectStore('gen_members').getAll();
        req.onsuccess = e => resolve(e.target.result);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genMembersDbAdd(record) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_members', 'readwrite').objectStore('gen_members').add(record);
        req.onsuccess = e => resolve(e.target.result);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genMemberDbPut(id, record) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const rec = Object.assign({}, record, { id });
        const req = d.transaction('gen_members', 'readwrite').objectStore('gen_members').put(rec);
        req.onsuccess = () => resolve(id);
        req.onerror = e => reject(e.target.error);
      }));
    }
    function genMemberDbDelete(id) {
      return initGenDB().then(d => new Promise((resolve, reject) => {
        const req = d.transaction('gen_members', 'readwrite').objectStore('gen_members').delete(id);
        req.onsuccess = () => resolve();
        req.onerror = e => reject(e.target.error);
      }));
    }
    ...
    /* ===================== 프로젝트 관리 ===================== */
    function onClickProjectSwitch() {
      requireAuthThen(openProjectModal, '프로젝트 전환은 관리자만 가능합니다.');
    }
    ...
    function openProjAddForm(editData) {
      document.getElementById('projAddFormTitle').textContent = editData ? '프로젝트 수정' : '새 프로젝트 추가';
      document.getElementById('pfEditId').value = editData ? editData.id : '';
      document.getElementById('pfName').value = editData ? (editData.name || '') : '';
      document.getElementById('projAddForm').classList.add('open');
      setTimeout(() => document.getElementById('pfName').focus(), 50);
    }

    function closeProjAddForm() {
      document.getElementById('projAddForm').classList.remove('open');
    }

    function saveProjForm() {
      const name = document.getElementById('pfName').value.trim();
      if (!name) { showToast('프로젝트명을 입력하세요.', 'err'); document.getElementById('pfName').focus(); return; }
      const editId = document.getElementById('pfEditId').value;
      const record = { name };
      const client = getSupabaseClient();
      if (!client) { showToast('Supabase 연결이 필요합니다.', 'err'); return; }

      const promise = editId
        ? client.from('wbs_gen_projects').update(record).eq('id', Number(editId)).select().single()
        : client.from('wbs_gen_projects').insert(record).select().single();

      promise.then(({ data, error }) => {
        if (error) throw error;
        if (!data) throw new Error('저장된 데이터를 가져오지 못했습니다.');
        showToast(editId ? '프로젝트가 수정되었습니다.' : '프로젝트가 추가되었습니다.', 'ok');
        closeProjAddForm();
        if (!editId) {
          selectProject(Array.isArray(data) ? data[0] : data);
        } else {
          const row = Array.isArray(data) ? data[0] : data;
          if (_currentProject && row && _currentProject.id === row.id) {
            _currentProject = row;
            updateProjectUI();
          }
          renderProjectList();
        ...
        if (msg.includes('does not exist') || msg.includes('relation')) {
          showToast('Supabase 테이블이 없습니다. supabase_setup_gen.sql 을 실행하세요.', 'err');
        } else {
          showToast('저장 오류: ' + msg, 'err');
        }
      });
    }
    ...
    function selectProject(project) {
      ...
      localStorage.setItem(GEN_PROJECT_KEY, String(project.id));
      updateUrlParam(null); // 다른 프로젝트의 ?id= 파라미터 제거
      updateProjectUI();
      closeProjModal();
      WBS_DATA = []; CURRENT_SOURCE_NAME = ''; CURRENT_FILE_BUFFER = null;
      TODAY = startOfDay(new Date());
      initFilters(); renderAll();
      updateHistoryCount(); populateFileSelect(); renderMembersPage();
      getCombinedHistoryList().then(all => {
        const latest = all
          .filter(r => r.wbsDate && parseDate(r.wbsDate))
          .sort((a, b) => b.wbsDate.localeCompare(a.wbsDate))[0];
        if (latest) restoreAnalysis(latest.id);
        else navigateTo('p-upload');
      }).catch(() => navigateTo('p-upload'));
    }

    function updateProjectUI() {
      const name = _currentProject ? _currentProject.name : '프로젝트 미선택';
      const el = document.getElementById('sidebarProjectName');
      if (el) el.textContent = name;
      const h1 = document.getElementById('topbarProjectName');
      if (h1) h1.textContent = _currentProject ? `${_currentProject.name} — WBS 일정 분석` : 'WBS 일정 분석';
      document.title = _currentProject ? `${_currentProject.name} | WBS 일정 분석` : 'WBS 일정 분석';
    }
    ...
    function deleteProjConfirm(id, name) {
      if (!confirm(`본 프로젝트를 삭제하면 프로젝트 내 연관 데이터(파일, 구성원 정보 등)가 전부 삭제됩니다. 진행하시겠습니까?`)) return;
      const client = getSupabaseClient();
      if (!client) { showToast('Supabase 연결이 필요합니다.', 'err'); return; }
      client.from('wbs_gen_projects').delete().eq('id', id).then(({ error }) => {
        if (error) { showToast('삭제 오류: ' + error.message, 'err'); return; }
        showToast('프로젝트가 삭제되었습니다.', '');
        if (_currentProject && _currentProject.id === id) {
          _currentProject = null;
          localStorage.removeItem(GEN_PROJECT_KEY);
          updateProjectUI();
        }
        renderProjectList();
      });
    ...
    document.addEventListener('keydown', ev => {
      if (ev.key === 'Escape' && document.getElementById('projOverlay').classList.contains('open')) closeProjModal();
    });
    ...
    Promise.all([initDB(), initGenDB()]).then(() => {
      updateSupabaseStatus();
      initSupabaseEvents();

      const client = getSupabaseClient();

      // 프로젝트가 로드된 후 분석 데이터를 복원하는 공통 함수
      function afterProjectLoaded(proj, initId) {
        _currentProject = proj;
        localStorage.setItem(GEN_PROJECT_KEY, String(proj.id));
        updateProjectUI();
        updateHistoryCount();
        populateFileSelect();
        renderMembersPage();

        const doLoadLatest = () => getCombinedHistoryList().then(all => {
          const latest = all
            .filter(r => r.wbsDate && parseDate(r.wbsDate))
            .sort((a, b) => {
        ...
        if (initId && client) {
          client.from('wbs_gen_analyses').select('id, project_id').eq('id', Number(initId)).maybeSingle()
            .then(({ data }) => {
              if (data && data.project_id === proj.id) restoreAnalysis(Number(initId));
              else { updateUrlParam(null); doLoadLatest(); }
            }).catch(() => doLoadLatest());
        } else {
          doLoadLatest();
        }
      ...
      // ?id= 파라미터로 분석 레코드를 역추적해 프로젝트 자동 로드 (인증 불요)
      function tryLoadByAnalysisId(initId, fallback) {
        if (!initId || !client) { fallback(); return; }
        client.from('wbs_gen_analyses').select('id, project_id').eq('id', Number(initId)).maybeSingle()
          .then(({ data }) => {
            if (!data || !data.project_id) { fallback(); return; }
            return client.from('wbs_gen_projects').select('*').eq('id', data.project_id).single()
              .then(({ data: proj, error }) => {
                if (error || !proj) { fallback(); return; }
                afterProjectLoaded(proj, initId);
              });
          }).catch(() => fallback());
      }

      // Supabase에서 첫 번째 프로젝트를 자동 로드 (프로젝트 미선택 브라우저용)
      function tryAutoLoadFirstProject(initId) {
        if (!client) { navigateTo('p-upload'); return; }
        client.from('wbs_gen_projects').select('*').order('id').limit(1)
          .then(({ data, error }) => {
            if (error || !data || data.length === 0) { navigateTo('p-upload'); return; }
            afterProjectLoaded(data[0], initId);
          }).catch(() => navigateTo('p-upload'));
      }

      const initId = new URLSearchParams(window.location.search).get('id');
      const savedProjId = localStorage.getItem(GEN_PROJECT_KEY);

      if (savedProjId) {
        const load = client
          ? client.from('wbs_gen_projects').select('*').eq('id', Number(savedProjId)).single()
              .then(({ data, error }) => { if (error || !data) throw new Error('not found'); return data; })
          : Promise.reject(new Error('no client'));

        load.then(proj => afterProjectLoaded(proj, initId))
          .catch(() => {
            localStorage.removeItem(GEN_PROJECT_KEY);
            // localStorage의 프로젝트가 없어진 경우 → ?id= 역추적 또는 첫 프로젝트 자동 로드
            tryLoadByAnalysisId(initId, () => tryAutoLoadFirstProject(initId));
          });
      } else {
        // 새 브라우저/미선택: ?id= 역추적 → 없으면 첫 프로젝트 자동 로드
        tryLoadByAnalysisId(initId, () => tryAutoLoadFirstProject(initId));
      }
    }).catch(() => {
      navigateTo('p-upload');
